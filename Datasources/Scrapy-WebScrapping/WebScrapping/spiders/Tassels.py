...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ["https://tassels.pk/collections/sale?page="+str(i) for i in range(1,200)]
    ...
    def parse(self, response):
        article_urls = response.xpath("//div[contains(@class,'product-img')]/a/@href").extract()
        for article_url in article_urls:
            article_url = "https://tassels.pk" + article_url
            yield scrapy.Request(article_url, callback=self.parse_dir_contents)

    def parse_dir_contents(self, response):
        items = WebscrappingItem()
        title = response.xpath("//h1[@class='product_title entry-title']/text()").extract_first().replace(r"\n","").strip()
        category = response.xpath('//div[@class="product attribute overview"]/div[@class="value"]/text()').extract()
        description = response.xpath("//div[@class='Shopify-product-details__short-description']/p/text()").extract_first()
        old_price = response.xpath(
            "//del[@id='old-product-price']/span/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//ins[@id='product-price']/span/text()").extract_first()
        image_links = response.xpath(
            "//div[contains(@class,'shopify-product-gallery__image')]/a/@href").extract()
