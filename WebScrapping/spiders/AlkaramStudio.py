...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ["https://www.alkaramstudio.com/collections/sale?page=" + str(i) for i in range(1, 500)]
        for url in urls:
    ...
    def parse(self, response):
        ...
        for article_url in article_urls:
            yield scrapy.Request('https://www.alkaramstudio.com'+article_url, callback=self.parse_dir_contents)
    ...
    def parse_dir_contents(self, response):
        ...
        old_price = response.xpath(
            "//div[@class='t4s-product-price']/del/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//div[@class='t4s-product-price']/ins/text()").extract_first()
        ...
        items["old_price_d"] = float(str(old_price).strip().replace("PKR", "").replace(",", "").strip(" "))
        items["final_price_d"] = float(str(final_price).strip().replace("PKR", "").replace(",", "").strip(" "))
