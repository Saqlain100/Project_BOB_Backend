...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ["https://hangten.com.pk/collections/sale?page=" + str(i) for i in range(1, 500)]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        article_urls = response.xpath("//*[@class='product-bottom']/a[@class='product-title change-text']/@href").extract()
        for article_url in article_urls:
            yield scrapy.Request('https://hangten.com.pk'+article_url, callback=self.parse_dir_contents)
