...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ['https://shahidafridistore.com/collections/sale?page=' + str(i) for i in range(1, 500)]
        for url in urls:
    ...
    def parse(self, response):
        ...
        for article_url in article_urls:
            yield scrapy.Request('https://shahidafridistore.com'+article_url, callback=self.parse_dir_contents)
    ...
    def parse_dir_contents(self, response):
        ...
        old_price = response.xpath(
            "//div[@class='product__info-wrapper grid__item']//s[@class='price-item price-item--regular']/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//div[@class='product__info-wrapper grid__item']//span[@class='price-item price-item--sale price-item--last']/text()").extract_first()
