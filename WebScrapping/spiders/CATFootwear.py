...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ["https://www.catfootwear.com.pk/collections/sale?page=" + str(i) for i in range(1, 500)]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        article_urls = response.xpath("//a[@class='product-card__link product-card__switchImage']/@href").extract()
        for article_url in article_urls:
            yield scrapy.Request('https://www.catfootwear.com.pk'+article_url, callback=self.parse_dir_contents)
    ...
    def parse_dir_contents(self, response):
        ...
        old_price = response.xpath(
            "//div[@class='product__price']//span[@class='price-item price-item--regular']/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//div[@class='product__price']//span[@class='price-item price-item--sale']/text()").extract_first()
        ...
        items["old_price_d"] = float(str(old_price).strip().replace("Rs.", "").replace(",", "").strip(" "))
        items["final_price_d"] = float(str(final_price).strip().replace("Rs.", "").replace(",", "").strip(" "))
