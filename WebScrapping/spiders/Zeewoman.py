...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ["https://zeenwoman.com/search?q=sale&page="+str(i) for i in range(1,200)]
    ...
    def parse(self, response):
        article_urls = response.xpath("//h3[@class='t4s-product-title']/a/@href").extract()
        for article_url in article_urls:
            article_url = "https://zeenwoman.com" + article_url
    ...
    def parse_dir_contents(self, response):
        ...
        title = response.xpath("//h1[@class='t4s-product__title']/text()").extract_first().replace(r"\n","").strip()
        category = response.xpath('//div[@class="product attribute overview"]/div[@class="value"]/text()').extract()
        description = ",".join(response.xpath("//div[contains(@class,'description')]/p/text()").extract())
        old_price = response.xpath(
            "//div[@class='t4s-product__info-container t4s-product__info-container--sticky']//del/span/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//div[@class='t4s-product__info-container t4s-product__info-container--sticky']//ins/span/text()").extract_first()
        temp_lst = []
        for item in response.xpath("//div[contains(@class,'product__media-wrapper')]//noscript").extract():
            temp_lst.append(item.split('src="')[1].split('" alt')[0])
        image_links = temp_lst.copy()
