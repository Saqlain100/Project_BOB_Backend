...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = ["https://www.mariab.pk/collections/all-sale?page=" + str(i) for i in range(1, 500)]
    ...
    def parse(self, response):
        ...
        for article_url in article_urls:
            article_url = "https://www.mariab.pk" + article_url
            yield scrapy.Request(article_url, callback=self.parse_dir_contents)

    def parse_dir_contents(self, response):
        items = WebscrappingItem()
        title = response.xpath("//h1[contains(@class,'product-single__title')]/text()").extract_first().replace(r"\n","").strip()
        ...
        if old_price == None:
            old_price = 0
        final_price = response.xpath('//span[@class="asasas product__price final-price on-sale"]/text()').extract_first()
