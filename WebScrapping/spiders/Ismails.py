...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ["https://www.ismailsclothing.com/collections/sale-1"]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        article_urls = response.xpath("//a[@class='full-unstyled-link']/@href").extract()
        for article_url in article_urls:
            yield scrapy.Request('https://www.ismailsclothing.com'+article_url, callback=self.parse_dir_contents)
