...
class QuotesSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for article_url in article_urls:
            article_url = "https://declarepakistan.com" + article_url
            yield scrapy.Request(article_url, callback=self.parse_dir_contents)

    def parse_dir_contents(self, response):
        items = WebscrappingItem()
        title = response.xpath("//h1[@class='h2 product-single__title']/text()").extract_first().replace(r"\n","").strip()
        category = response.xpath('//div[@class="product attribute overview"]/div[@class="value"]/text()').extract()
        description = ",".join(response.xpath("//div[@class='product-block']/div[@class='rte']/p/text()").extract())
        old_price = response.xpath(
            "//span[@class='product__price product__price--compare']/span/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//span[@class='product__price sale-price']/span/text()").extract_first()
        image_links = response.xpath(
            "//div[@class='image-wrap']/a/@href").extract()
