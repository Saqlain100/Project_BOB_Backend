...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ['https://www.polkadots.pk/collections/new-sale?page=' + str(i) for i in range(1, 500)]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        article_urls = response.xpath("//div[@class='product-item-buttons']/a/@href").extract()
    ...
    def parse_dir_contents(self, response):
        ...
        old_price = response.xpath(
            "//del[@class='product-price-old']/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//span[@class='product-price']/text()").extract_first()
