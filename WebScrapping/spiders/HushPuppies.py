...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ["https://www.hushpuppies.com.pk/collections/sale?page=" + str(i) for i in range(1, 500)]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        article_urls = response.xpath("//div[@class='wrapper-button-card']/a[contains(@class,'product-title')]/@href").extract()
        for article_url in article_urls:
            yield scrapy.Request('https://www.hushpuppies.com.pk'+article_url, callback=self.parse_dir_contents)

    def parse_dir_contents(self, response):
        items = WebscrappingItem()
        items["body"] = response.text
        logging.info(items["body"])
        title = response.xpath("//meta[@name='twitter:title']/@content").extract_first().replace(r"\n", "").strip()
