...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ["https://lakhanyonline.com/collections/sale?page=" + str(i) for i in range(1, 500)]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        article_urls = response.xpath("//*[contains(@class,'product-item__product-title')]/a/@href").extract()
        for article_url in article_urls:
            yield scrapy.Request('https://lakhanyonline.com'+article_url, callback=self.parse_dir_contents)
    ...
    def parse_dir_contents(self, response):
        ...
        old_price = response.xpath(
            "//div[@class='product__price-and-ratings']//s/span[@class='money']/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//div[@class='product__price-and-ratings']//span/span[@class='money']/text()").extract_first()
