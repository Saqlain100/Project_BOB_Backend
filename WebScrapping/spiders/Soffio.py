...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = ["https://soffio.pk/collections/sale?page=" + str(i) for i in range(1, 200)]
    ...
    def parse(self, response):
        article_urls = response.xpath("//div[@class='product-image image-swap']/a/@href").extract()
        for article_url in article_urls:
            article_url = "https://soffio.pk" + article_url
    ...
    def parse_dir_contents(self, response):
        ...
        description = ",".join(response.xpath("//div[@class='tab-content']/div/p/text()").extract())
        old_price = response.xpath(
            "//div[@class='prices']/span[contains(@class,'compare-price')]/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//div[@class='prices']/span[contains(@class,'price on-sale')]/text()").extract_first()
