...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ["https://www.kayseria.com/collections/sale?page="+str(i) for i in range(1,200)]
    ...
    def parse(self, response):
        ...
        for article_url in article_urls:
            article_url = "https://www.kayseria.com" + article_url
            yield scrapy.Request(article_url, callback=self.parse_dir_contents)

    def parse_dir_contents(self, response):
        items = WebscrappingItem()
        title = response.xpath("//h1/span/text()").extract_first().replace(r"\n","").strip()
        category = response.xpath('//div[@class="product attribute overview"]/div[@class="value"]/text()').extract()
        description = response.xpath("//div[@class='productView-desc halo-text-format']/text()").extract_first()
        old_price = response.xpath(
            "//div[contains(@class,'productView-container')]//s[@class='price-item price-item--regular']/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//div[contains(@class,'productView-container')]//span[@class='price-item price-item--sale']/text()").extract_first()
