...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ["https://www.thefabricstore.pk/collections/sale?page="+str(i) for i in range(1,200)]
    ...
    def parse(self, response):
        article_urls = response.xpath("//div[@class='ProductItem__Wrapper']/a/@href").extract()
        for article_url in article_urls:
            article_url = "https://www.thefabricstore.pk" + article_url
            yield scrapy.Request(article_url, callback=self.parse_dir_contents)

    def parse_dir_contents(self, response):
        items = WebscrappingItem()
        title = response.xpath("//h1[@class='ProductMeta__Title Heading u-h2']/text()").extract_first().replace(r"\n","").strip()
        category = response.xpath('//div[@class="product attribute overview"]/div[@class="value"]/text()').extract()
        description = ",".join(response.xpath("//div[@class='ProductMeta__Description']//ul/li/text()").extract())
        old_price = response.xpath(
            "//div[@class='ProductMeta__PriceList Heading']/span[contains(@class,'ProductMeta__Price Price Price--compareAt')]/span/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//div[@class='ProductMeta__PriceList Heading']/span[contains(@class,'ProductMeta__Price Price Price--highlight')]/span/text()").extract_first()
        image_links = response.xpath(
            "//div[@class='Product__SlideshowNavScroller']/a/@href").extract()
