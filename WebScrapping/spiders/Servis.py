...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ['https://www.servis.pk/search?q=sale&page=' + str(i) for i in range(1, 500)]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        article_urls = response.xpath("//li[@class='collection__grid-item']/div/a/@href").extract()
        for article_url in article_urls:
            yield scrapy.Request('https://www.servis.pk'+article_url, callback=self.parse_dir_contents)
    ...
    def parse_dir_contents(self, response):
        ...
        old_price = response.xpath(
            "//s[@class='price-item price-item--compare']/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//span[@class='price-item price-item--sale']/text()").extract_first()
