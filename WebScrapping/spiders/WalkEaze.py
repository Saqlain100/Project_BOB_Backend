...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ['https://walkeaze.com/collections/sale?page=' + str(i) for i in range(1, 500)]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        article_urls = response.xpath("//div[contains(@class,'product-collection__title')]/*/a/@href").extract()
        for article_url in article_urls:
            yield scrapy.Request('https://walkeaze.com'+article_url, callback=self.parse_dir_contents)
    ...
    def parse_dir_contents(self, response):
        ...
        old_price = response.xpath(
            "//div[contains(@class,'product-page-info__price')]//span[@class='money']/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("(//div[contains(@class,'product-page-info__price')]//span[@class='money'])[2]/text()").extract_first()
        ...
        items["old_price_d"] = float(str(old_price).strip().replace("PKR", "").replace("₨.", "").replace(",", "").strip(" "))
        items["final_price_d"] = float(str(final_price).strip().replace("PKR", "").replace("₨.", "").replace(",", "").strip(" "))
