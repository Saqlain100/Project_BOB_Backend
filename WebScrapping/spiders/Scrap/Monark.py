...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ['https://monark.com.pk/collections/sale?usf_sort=-date&usf_take=' + str(i) for i in range(0, 4000,28)]
        for url in urls:
    ...
    def parse(self, response):
        ...
        for article_url in article_urls:
            yield scrapy.Request('https://monark.com.pk'+article_url, callback=self.parse_dir_contents)
    ...
    def parse_dir_contents(self, response):
        ...
        old_price = response.xpath(
            "//div[@class='t4s-product__price-review']/div[@class='t4s-product-price']/del/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//div[@class='t4s-product__price-review']/div[@class='t4s-product-price']/ins/text()").extract_first()
