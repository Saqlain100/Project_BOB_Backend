...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ["https://lamosaik.com/collections/sale?section_id=collection_page&page="+str(i) for i in range(1,200)]
    ...
    def parse(self, response):
        article_urls = response.xpath("//a[@class='cd chp']/@href").extract()
        for article_url in article_urls:
            article_url = "https://lamosaik.com/" + article_url
    ...
    def parse_dir_contents(self, response):
        ...
        description = ','.join(response.xpath("//p[@class='pText']/span/text()").extract())
        old_price = response.xpath(
            "//p[@class='price_range']/del/span/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//p[@class='price_range']/ins/span/text()").extract_first()
