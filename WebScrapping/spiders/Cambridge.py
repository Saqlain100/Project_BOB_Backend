...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ["https://thecambridgeshop.com/collections/sale-all?page=" + str(i) for i in range(1, 500)]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        article_urls = response.xpath("//a[@class='t4s-full-width-link']/@href").extract()
        for article_url in article_urls:
            yield scrapy.Request('https://thecambridgeshop.com'+article_url, callback=self.parse_dir_contents)
    ...
    def parse_dir_contents(self, response):
        ...
        old_price = response.xpath(
            "//div[@id='product_info_wrapper']//p[@class='original-price']/span/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//div[@id='product_info_wrapper']//p[@class='sale-price']/span/text()").extract_first()
