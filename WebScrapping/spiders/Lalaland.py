...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ["https://www.lalaland.pk/sale?gender_filter=eyJnZW5kZXIiOlsiMSJdfQ&page=" + str(i) for i in range(1, 200)]
    ...
    def parse(self, response):
        article_urls = response.xpath("//a[@class='product_url']/@href").extract()
    ...
    def parse_dir_contents(self, response):
        ...
        title = response.xpath("//h1[@class='detail_sub_heaading']/text()").extract_first().replace(r"\n","").strip()
        ...
        old_price = response.xpath(
            "//del[@id='discounted_price']/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//span[@id='price_range']/text()").extract_first()
