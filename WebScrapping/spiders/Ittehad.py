...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ["https://www.ittehadtextiles.com/collections/sale?page=" + str(i) for i in range(1, 500)]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        article_urls = response.xpath("//*[@class='t4s-product-title']/a/@href").extract()
        for article_url in article_urls:
            yield scrapy.Request('https://www.ittehadtextiles.com'+article_url, callback=self.parse_dir_contents)
    ...
    def parse_dir_contents(self, response):
        ...
        old_price = response.xpath(
            "//div[@class='t4s-product__price-review']/div[@class='t4s-product-price']/del/span/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//div[@class='t4s-product__price-review']/div[@class='t4s-product-price']/ins/span/text()").extract_first()
