...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ["https://engine.com.pk/collections/women-sale?page="+str(i) for i in range(1,200)]
    ...
    def parse(self, response):
        article_urls = response.xpath("//div[@class='product-bottom']/a/@href").extract()
        for article_url in article_urls:
            article_url = "https://engine.com.pk" + article_url
    ...
    def parse_dir_contents(self, response):
        ...
        description = response.xpath("//h1[@class='product-title']/span/text()").extract_first()
        old_price = response.xpath(
            "//div[@class='prices']/span[@class='compare-price']/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//div[@class='prices']/span[@class='price on-sale']/text()").extract_first()
