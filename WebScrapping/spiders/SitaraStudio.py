...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ['https://sitarastudio.pk/search?q=sale&page=' + str(i) for i in range(1, 500)]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        article_urls = response.xpath("//a[contains(@class,'product-title')]/@href").extract()
        for article_url in article_urls:
            yield scrapy.Request('https://sitarastudio.pk'+article_url, callback=self.parse_dir_contents)
