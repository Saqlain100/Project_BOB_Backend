...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ['https://www.sokamal.com/collections/sale?page=' + str(i) for i in range(1, 500)]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        article_urls = response.xpath("//a[contains(@class,'grid-view-item__link')]/@href").extract()
        for article_url in article_urls:
            yield scrapy.Request('https://www.sokamal.com'+article_url, callback=self.parse_dir_contents)
