...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ['https://originsestore.com/collections/sale?page=' + str(i) for i in range(1, 500)]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        article_urls = response.xpath("//*[@class='ProductItem__Title Heading']/a/@href").extract()
        for article_url in article_urls:
            yield scrapy.Request('https://originsestore.com'+article_url, callback=self.parse_dir_contents)
    ...
    def parse_dir_contents(self, response):
        ...
        old_price = response.xpath(
            "//div[@class='ProductMeta__PriceList Heading']/span[contains(@class,'Price Price--compareAt')]/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//div[@class='ProductMeta__PriceList Heading']/span[contains(@class,'Price Price--highlight')]/text()").extract_first()
