...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ["https://akgalleria.com/catalogsearch/result/index/?q=sale&p=" + str(i) for i in range(1, 500)]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        article_urls = response.xpath("//div[@class='product-top']/a/@href").extract()
    ...
    def parse_dir_contents(self, response):
        ...
        old_price = response.xpath(
            "//meta[@itemprop='highPrice']/@content").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//meta[@itemprop='lowPrice']/@content").extract_first()
