...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ['https://shaposh.pk/index.php?fc=module&module=leoproductsearch&controller=productsearch&search_query=sale&page=' + str(i) for i in range(1, 500)]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        article_urls = response.xpath("//*[contains(@class,'product-title')]/a/@href").extract()
    ...
    def parse_dir_contents(self, response):
        ...
        old_price = response.xpath(
            "//span[@class='current-price']//span[@class='regular-price']/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//span[@class='current-price']/span[@itemprop='price']/text()").extract_first()
