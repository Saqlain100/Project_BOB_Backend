...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ['https://royaltag.com.pk/collections/sale?page=' + str(i) for i in range(1, 500)]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        article_urls = response.xpath("//div[contains(@class,'card-information__wrapper')]/a/@href").extract()
        for article_url in article_urls:
            yield scrapy.Request('https://royaltag.com.pk'+article_url, callback=self.parse_dir_contents)
    ...
    def parse_dir_contents(self, response):
        ...
        old_price = response.xpath(
            "//div[@class='price price--medium price--on-sale ']//s[@class='price-item price-item--regular']/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//div[@class='price price--medium price--on-sale ']//span[@class='price-item price-item--regular']/text()").extract_first()
