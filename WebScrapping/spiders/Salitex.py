...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ['https://www.salitexonline.com/collections/sale?page=' + str(i) for i in range(1, 500)]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        article_urls = response.xpath("//a[@class='product-card__link']/@href").extract()
        for article_url in article_urls:
            yield scrapy.Request('https://www.salitexonline.com'+article_url, callback=self.parse_dir_contents)
    ...
    def parse_dir_contents(self, response):
        ...
        old_price = response.xpath(
            "//s[@class='price__sale-compare money']/span/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//span[@class='price__sale-value money']/span/text()").extract_first()
