...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = ["https://diners.com.pk/collections/sale?page="+str(i) for i in range(1,200)]
    ...
    def parse(self, response):
        article_urls = response.xpath("//a[@class='card-link']/@href").extract()
        for article_url in article_urls:
            article_url = "https://diners.com.pk" + article_url
            yield scrapy.Request(article_url, callback=self.parse_dir_contents)

    def parse_dir_contents(self, response):
        items = WebscrappingItem()
        title = response.xpath("//h1[@class='productView-title']/span/text()").extract_first().replace(r"\n","").strip()
        category = response.xpath('//div[@class="product attribute overview"]/div[@class="value"]/text()').extract()
        description = response.xpath("//h1[@class='productView-title']/span/text()").extract_first()
        old_price = response.xpath(
            "//div[@class='productView-moreItem']//s[@class='price-item price-item--regular']/span/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//div[@class='productView-moreItem']//span[@class='price-item price-item--sale']/span/text()").extract_first()
        image_links = response.xpath(
            "//div[@class='media']/@href").extract()
        ...
        items["old_price_d"] = float(str(old_price).replace("₨","").replace(",","").strip(" "))
        items["final_price_d"] = float(str(final_price).replace("₨", "").replace(",","").strip(" "))
