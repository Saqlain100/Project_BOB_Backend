...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = ["https://outfitters.com.pk/collections/women-special-prices?page=" + str(i) for i in range(1, 2000)]
    ...
    def parse(self, response):
        article_urls = response.xpath("//ul[@id='swatch-select-option']/li[1]/@data-url").extract()
        for article_url in article_urls:
            article_url = "https://outfitters.com.pk" + article_url
            yield scrapy.Request(article_url, callback=self.parse_dir_contents)

    def parse_dir_contents(self, response):
        items = WebscrappingItem()
        title = response.xpath("//div[@class='product__title']/h1/text()").extract_first().replace(r"\n","").strip()
        category = response.xpath('//div[@class="product attribute overview"]/div[@class="value"]/text()').extract()
        description = ",".join(response.xpath("//div[contains(@class,'product__description rte quick-add-hidden desktop')]/p/text()").extract())
        old_price = response.xpath(
            "//div[@class='product__info-wrapper grid__item']//s[@class='price-item price-item--regular']/span/text()").extract_first()
        if old_price == None:
            old_price = 0
        final_price = response.xpath("//div[@class='product__info-wrapper grid__item']//span[@class='price-item price-item--sale price-item--last']/span[@class='money']/text()").extract_first()
